window.addEventListener('DOMContentLoaded', function(){
    let tabs = document.querySelectorAll('.tabheader__item'),
        ...
        new MenuCard(
            ...
            ' молоко из миндаля, овса, кокоса или гречки, правильные количество белков за счет тофу и импортных ' +
        ...
        if (slides.length < 10) {
            total.textContent = `0${slideIndex.length}`;
        ...
        slidesField.style.display = '0.5s all';

        slidesWrapper.styles.overflow = 'hidden';

        slides.forEach(slide => {
            slide.style.width = width;
        });

        slider.style.positiom = 'relative';
        ...
        dots.forEach(dot => {
            dot.addEventListener('click', (e) => {
                ...
                dots.forEach(dot => dot.style.opacity = ".5");
                dots[slideIndex-1].styles.opacity = 1;
            });
        });
